...
def get_document_info(path: str) -> str:
    ...
    count, styles used, and core properties (author, title, subject, etc.).
    ...
    info = {
        ...
        "tables": [
            {"table_index": i, "rows": len(t.rows), "columns": len(t.columns)}
...
def list_paragraphs(path: str, start: int = 0, end: Optional[int] = None) -> str:
    """
